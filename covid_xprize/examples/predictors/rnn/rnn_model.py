# Adapted from abudhkar/covid-xprize/ken/covid_xprize_pipeline/rnn_model.py
...
class Simple_RNN(pl.LightningModule):
    ...
    def __init__(self, nb_context, nb_action, nb_static, nb_lookback, train_data_size=1, val_data_size=1):
        super().__init__()
        rnn_context_hidden_size = nb_context
        ...
        self.rnn_context = torch.nn.RNN(input_size=nb_context, hidden_size=rnn_context_hidden_size, num_layers=4)
        self.rnn_action = torch.nn.RNN(input_size=nb_action, hidden_size=rnn_action_hidden_size, num_layers=4)
        
        self.linear_context = torch.nn.Linear(rnn_context_hidden_size, 1)
        self.linear_action = torch.nn.Linear(rnn_action_hidden_size, 1)
        
        self.linear_static = torch.nn.Linear(nb_static, 20)
        
        self.linear_combine_contexdt_action = torch.nn.Linear(nb_lookback*2, 10)
        
        self.linear_static_others = torch.nn.Linear(30, 1)
        
        self.l1_loss = torch.nn.L1Loss(reduction="mean")

    def predict(self, x_context, x_action, x_static):
        ...
        static_out = self.linear_static(x_static)
        
        other_out = self.linear_combine_contexdt_action(torch.cat((context_out, action_out), dim=1))
        
        final_out = self.linear_static_others(torch.cat((static_out, other_out), dim=1))
        final_out = final_out.squeeze(-1)
        return final_out

    def forward(self, x_context, x_action, x_static, y):
        y_hat = self.predict(x_context, x_action, x_static)
        loss = self.l1_loss(y, y_hat)
        return loss

    def training_step(self, batch, batch_idx):
        x_context, x_action, x_static, y = batch[0], batch[1], batch[2], batch[3]
        train_loss = self(x_context, x_action, x_static, y)
    ...
    def validation_step(self, batch, batch_idx):
        x_context, x_action, x_static, y = batch[0], batch[1], batch[2], batch[3]
        val_loss = self(x_context, x_action, x_static, y)
        self.log("validation_loss", val_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.val_data_size += 1
        return val_loss

    def test_step(self, batch, batch_idx):
        x_context, x_action, x_static, y = batch[0], batch[1], batch[2], batch[3]
        loss = self(x_context, x_action, x_static, y)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        for output in validation_step_outputs:
            avg_val_loss += output
        avg_val_loss = avg_val_loss / len(validation_step_outputs)
