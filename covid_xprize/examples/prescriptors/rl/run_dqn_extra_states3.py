...
env = gym.make(task, country=country, IP_history_file=IP_FILE, prescription_temp_file="prescription_temp_gamma95_dqn.csv", prediction_temp_file="predictions/prediction_temp_gamma95_dqn.csv")


# Hyperparameters
lr, epoch, batch_size = 1e-2, 2, 64
train_num, test_num = 1, 1
gamma, n_step, target_freq = 0.95, 3, 320
buffer_size = 20000
eps_train, eps_test = 0.1, 0.05
step_per_epoch, collect_per_step = 4, 10
writer = SummaryWriter('dqn-agent-results/dqn3_gamma95')


# Make environments
train_envs = ts.env.DummyVectorEnv([lambda: gym.make(task, country=country, IP_history_file=IP_FILE, prescription_temp_file="prescription_temp_gamma95_dqn.csv", prediction_temp_file="predictions/prediction_temp_gamma95_dqn.csv") for _ in range(train_num)])
test_envs = ts.env.DummyVectorEnv([lambda: gym.make(task, country=country, IP_history_file=IP_FILE, prescription_temp_file="prescription_temp_gamma95_dqn.csv", prediction_temp_file="predictions/prediction_temp_gamma95_dqn.csv") for _ in range(test_num)])
