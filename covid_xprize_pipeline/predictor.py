...
class XPrizePredictor(object):
    ...
    def _create_country_samples(df: pd.DataFrame, geos: list, new_cols=None) -> dict:
        ...
        if new_cols is None:
            new_cols = []
        context_columns = ['PredictionRatio'] + new_cols
    ...
    def train(self):
        ...
        for t in range(NUM_TRIALS):
            ...
            test_dataset = TensorDataset(test_X_context, test_X_action, test_y)
            ...
            test_dataloader = DataLoader(test_dataset, num_workers=4,
                                         batch_size=BATCH_SIZE)
            model = Simple_RNN(nb_context=nb_context, nb_action=nb_action,
                               nb_lookback=NB_LOOKBACK_DAYS,
    ...
    def _lstm_get_test_rollouts(self, model, df, top_geos, country_samples):
        ...
        for g in top_geos:
            ...
            temp = model.predict(test_X_context, test_X_action).detach().numpy()
